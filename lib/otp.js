...
export async function verifyOtp(tenantId, rawPhone, code) {
  ...
  rateLimit(`otpv:${phone}`, { limit: 12, windowMs: 600_000 }); // cheap first line; DB counter is the real cap
  ...
  if (!ok) {
    ...
    await prisma.user.update({
      where: { id: user.id },
      // At the cap, invalidate the code entirely — the attacker must request a new
      // one (rate-limited to 3/10min), capping total guesses per code at 5.
      data: attempts >= MAX_OTP_ATTEMPTS ? { otpHash: null, otpExpires: null, otpAttempts: 0 } : { otpAttempts: attempts },
    });
    return null;
  }
